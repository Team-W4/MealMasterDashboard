...
type UseArrayBase<T = any> = [T[], UseArrayActions<T>];

function useArrayBase<T = any>(initial: T[]): UseArrayBase<T> {
  ...
  const move = useCallback(
    (from: number, to: number) => setValue((it) => {
        const copy = it.slice();
  ...
  const modifyById = useCallback(
    // @ts-ignore not every array that you will pass down will have object with id field.
    (id, newValue) => setValue((arr) => arr.map((v) => (v.id === id ? { ...v, ...newValue } : v))),
    [],
  );
